...
The way I do it is: I run the command in [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Compilation-Mode.html][Emacs compilation mode]], so the output is sitting in an Emacs buffer. Then for each output message, I run the Emacs command =how-many=, aka =count-matches= (possibly with regexp matching) and write down the numbers.
