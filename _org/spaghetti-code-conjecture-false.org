...
The *[[https://nickdrozd.github.io/2021/01/26/spaghetti-code-conjecture.html][Spaghetti Code Conjecture]]* says that *Busy Beaver* programs ought to be, in some sense, *as complicated as possible*. *[[https://www.scottaaronson.com/blog/?p=4916][Scott Aaronson]]* puts it this way:
...
First, consider the /naive/ way to search over Turing machine programs of some class, namely just *enumerating all of them*. This gets infeasiable fast. My [[https://nickdrozd.github.io/2020/08/13/beeping-busy-beavers.html][early *Beeping Busy Beaver* results]] actually came from this approach, and it was miserable. There are just too many programs to deal with, and many of them are either duplicates or junk.
...
|------------------+-------+--------+------------|
| Naive            |  2^30 |   2^14 | 2,819      |
| Graph decoration |  2^24 |   2^21 | 66,349     |
| Tree generation  |  2^17 |   2^28 | 32,779,478 |
|------------------+-------+--------+------------|
...
The Strongly Connected Graph Conjecture is just one piece of the broader Spaghetti Code Conjecture. I've argued that the former is *true*, and I claim now that the latter might be *false*. To see why, consider the *[[https://cs.nyu.edu/pipermail/fom/2021-July/022743.html][4-state 2-color Beeping Busy Beaver champion]]*:
...
[[/assets/2021-09-25-spaghetti-code-conjecture-false/bbb-4-champion.png]]
...
One interpretation of the Spaghetti Code Conjecture says that a Busy Beaver graph ought to be featureless. But this program has a variety of distinctive features. Just from *eyeballing the graph* one can tell the roles of the different nodes:
  ...
  - Nodes =B= and =C= each execute while-loops on =1=. Since there are only ever finitely many marks the tape, these loops are *[[https://nickdrozd.github.io/2021/09/02/new-pylint-checks.html][guaranteed to terminate]]*.
...
So if it isn't complicated spaghetti code, then how does it run for so long? *[[https://www.sligocki.com/2021/07/17/bb-collatz.html][Shawn Ligocki]]* figured out that the program implements the following *Collatz-like function* (/L/: ℕ -> ℕ):
...
This is reminiscent of the lesson of the *[[https://www.scottaaronson.com/writings/bignumbers.html][Bigger Number Game]]*:
...
The first runner-up was submitted by none other than *[[http://turbotm.de/~heiner/BB/mabu90.html][Heiner Marxen]]*, who co-discovered the BB(5) champion in 1989. This entry implements a variant of the *Goodstein function*, a total function that Peano arithmetic can't prove to be total.

So the Bigger Number Game and the Bignum Bakeoff both suggest that Busy Beaver champions will win by theory rather than by tricks, and the BBB(4) champion program confirms it. Maybe the Spaghetti Code Conjecture is false. But what if this is *unjustified extrapolation*, and these early Busy Beaver results can't tell us anything about later ones? As *[[http://timothychow.net/][Timothy Chow]]* says:
...
This program was discovered by *[[https://github.com/boydjohnson/lin-rado-turing/][Boyd Johnson]]*. It runs for *158491 steps* before entering in [[https://nickdrozd.github.io/2021/02/24/lin-recurrence-and-lins-algorithm.html][Lin recurence]] with period of *17620 steps*. Its control flow graph is a *living nightmare*:

[[/assets/2021-09-25-spaghetti-code-conjecture-false/boyd-spaghetti.png]]
